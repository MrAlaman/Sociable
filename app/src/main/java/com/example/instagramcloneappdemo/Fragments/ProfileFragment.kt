...
import com.example.instagramcloneappdemo.Activity.AccountSettingsActivity
import com.example.instagramcloneappdemo.Activity.ShowUsersActivity
import com.example.instagramcloneappdemo.Adapter.MyImagesAdapter
import com.example.instagramcloneappdemo.Model.Post
import com.example.instagramcloneappdemo.Model.User
...
class ProfileFragment : Fragment() {
    ...
    private fun checkFollowAndFollowingButtonStatus()
    {
        ...
        val followingfRef = firebaseUser?.uid.let { it1 ->
            FirebaseDatabase.getInstance().reference
                .child("Follow").child(it1.toString())
                .child("Following")
        }

        if (followingfRef != null)
        {
        ...
        usersRef.addValueEventListener(object : ValueEventListener{
            ...
            override fun onDataChange(p0: DataSnapshot)
            {
                if (p0.exists())
                {
                    ...
                    Picasso.get().load(user!!.getImage()).placeholder(R.drawable.profile).into(view?.pro_image_profile_flag)
